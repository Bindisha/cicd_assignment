pipeline {
    ...
    environment {
        ...
        ECR_REPO_NAME = 'cicd/cicd_assignment'
            ...
            steps {
                script {
                    if (env.BRANCH_NAME == 'main') {
                        ...
                        checkout([$class: 'GitSCM', branches: [[name: "*/${params.BRANCH_NAME}"]], userRemoteConfigs: [[url: 'https://github.com/Bindisha/cicd_assignment.git']]])
                    }
                }
            }
        }
        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                withSonarQubeEnv('SonarQube') {
                    ...
                    -Dsonar.projectKey=HSM-key \
                    -Dsonar.projectName=HSM \
            ...
            steps {
                script {
                    ...
                    ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${EC2_USER}@${EC2_HOST} << 'EOF'
                        ...
                        sudo docker stop cicd_assignment || true
                        sudo docker rm cicd_assignment || true

                        echo "Running new container..."
                        sudo docker run -d --name cicd_assignment -p ${PORT_NO}:8080 --restart always \
